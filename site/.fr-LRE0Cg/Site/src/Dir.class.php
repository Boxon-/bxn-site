...
class Dir extends Component {
	...
	public function scan($ext=NULL,$name=NULL){
		$this->Log("scannig dir ".$this->url."...",'process');
		$output=array();
		$dirsToScan = array();
		$dirsScanned = array();
		$dirCount=1;
		$extCount=0;
		array_push($dirsToScan,$this->_scandir($this->url,'a'));	
				...
				foreach ($dirScan['f'] as $file){
					if($nFile = new File($dirScan['path'].$file)){
						...
						if($name==NULL&&$ext==NULL){
							...
							if($name!=NULL&&$ext==NULL){
								if($nFile->extractNakedName()==$name){
									array_push($output,$nFile);
									$extCount++;
								}						
							}
							if($name==NULL&&$ext!=NULL){
								if($nFile->extractFormat()==$ext){
									array_push($output,$nFile);
									$extCount++;
								}						
							}
							if($name!=NULL&&$ext!=NULL){
								if($nFile->extractNakedName()==$name&&$nFile->extractFormat()==$ext){
									array_push($output,$nFile);
									$extCount++;
								}							
							}
		...
		if($extCount==0){
			$this->Log("no files.".$ext."in this directory",'error');
		}
	...
	private function _scandir( $path = NULL , $option = NULL, $encodage = NULL ){
